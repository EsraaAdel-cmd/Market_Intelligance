...
if page == "Overview":
    ...
    for col,label,val,sub in [
        (k1,"Total Books",f"{total_f:,}",f"of {total_all:,} total"),
    ...
    st.markdown("<div style='margin-bottom:24px;'><span class='page-title'>AI Analyst</span><span class='live-dot'></span><div class='page-sub'>Ask anything about the market data in natural language</div></div>", unsafe_allow_html=True)
    top5_cats = filtered["category"].value_counts().head(5).to_dict()
    top5_price = filtered.groupby("category")["price"].mean().sort_values(ascending=False).head(5).to_dict()
    rating_dist = filtered["rating"].value_counts().sort_index().to_dict()
    ...
    prompts = {"Best value":"What are the best value books with high rating and low price?","Price insights":"Which categories have highest average prices?","Market summary":"Give me an executive summary of this books market.","Rating patterns":"What patterns exist in the rating distribution?"}
    if "quick_input" not in st.session_state:
        st.session_state.quick_input = ""
    for col,(label,prompt) in zip([q1,q2,q3,q4],prompts.items()):
        with col:
    ...
    with col_in:
        user_input = st.text_input("",value=st.session_state.quick_input,placeholder="Ask anything about the market data...",label_visibility="collapsed",key="chat_input")
    with col_btn:
        send = st.button("Send",use_container_width=True)
    if send and user_input.strip():
        st.session_state.quick_input = ""
        with st.spinner("Analyzing..."):
            reply = ask_groq(user_input,data_context)
